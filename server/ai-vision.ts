        ...
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: `请仔细观察这张图片，计算图片中独立包装的时尚饰品数量。

识别要点：
1. 每个饰品都装在一个独立的透明塑料包装袋中
2. 每个包装袋内有一个白色展示卡（带有"Fashion Jewelry"或类似文字）
3. 每个包装袋顶部有一个圆形挂孔
4. 请数清楚所有独立的包装袋数量，不要重复计数
5. 反光、阴影或包装袋的折痕不应被计算在内

重要：请确保只计算完整的、独立的包装袋数量。如果不确定，宁可少数不要多数。

请只返回一个数字，表示饰品包装袋的总数量。`,
              },
              {
                type: "image_url",
                image_url: {
